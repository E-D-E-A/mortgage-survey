...
export interface VarMeta {
  ...
  /**
   * ערך → מכסה: כמה משיבים שסיימו מותרים עם הערך הזה. ערך שאינו במפה — בלי
   * הגבלה, ולכן "בלי הגבלה" הוא היעדר רשומה ולא 0 (0 הוא מכסה אמיתית: תא
   * שנסגר). המכסה יושבת ליד התוויות ולא במפה נפרדת ברמת השאלון, כי היא תכונה
   * של הערך — כל מה שידוע על "זוג צעיר" נמצא במקום אחד.
   *
   * ⚠ ההגדרה בלבד. הספירה והניתוב חיים ב-engine/quota.ts ובקצה quota-get.
   */
