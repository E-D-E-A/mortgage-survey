// רשימת השאלונים בקונסולה: טעינה ופעולות ניהול (יצירה, שינוי שם, ארכוב,
...
function describe(e: unknown, action: 'create' | 'delete' | 'other'): string {
  if (e instanceof ApiError) {
    ...
    if (e.status === 409 && action === 'delete') {
      return 'לשאלון יש גרסאות שפורסמו ולכן אי אפשר למחוק אותו — אפשר לארכב אותו במקום';
    }
    if (e.status === 404) return 'השאלון לא נמצא — ייתכן שנמחק. רעננו את הרשימה.';
    if (e.status === 400) return 'הפרטים לא תקינים — בדקו את השם ואת המזהה';
    return `הפעולה נכשלה (${e.status}) — נסו שוב`;
  }
