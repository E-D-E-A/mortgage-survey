...
npm test           # כל הבדיקות (ראו "בדיקות" למטה)
```
...
`npm test` (vitest) מריץ הכל — גם קוד הדפדפן וגם ה-Netlify Functions. שתי השאלות
שהבדיקות אמורות לענות עליהן, ואיפה כל אחת נענית:

**1. התשובות נשמרות ב-DB, נכון ובמבנה הנכון**

| קובץ | מה נבדק |
| --- | --- |
| `src/data/journey.test.tsx` | משיב אמיתי (React + DOM) לוחץ על התוויות בעברית, וכל שורה שיוצאת ברשת נבדקת: סוג, סדר, מבנה ה-payload, ערכים מול מזהי האפשרויות, 15 שאלות, גיזום מקטע נטוש, רענון באמצע, retry על כשל שרת, ומגבלת האצווה |
| `netlify/functions/__tests__/events.test.ts` | הפונקציה עצמה: whitelist שדות, enum, מגבלות גודל, מסלול ה-dedup, ומה שנדחה לא מגיע ל-DB |
| `src/data/events.test.ts` | סינכרון שלוש שכבות סוגי האירועים (לקוח / פונקציה / `check` ב-SQL) והתאמת השורה לעמודות `survey_events` |

הבדיקות מאמתות גם את מה שה-views ב-`schema.sql` נשענים עליו: `payload.ms` בכל שורת
`answer` (בשביל `screen_funnel`) ו-`answers`/`vars` בשורת הסיום (בשביל
`completed_responses`).

**2. השאלון שמתפרסם הוא זה שרואים בקונסולה**

| קובץ | מה נבדק |
| --- | --- |
| `src/admin/flow-fidelity.test.ts` | כל מעבר שהמנוע יכול לבצע מופיע כקשת בתרשים — על כל צירופי הניתוב של השאלון האמיתי, ועל 120 שאלונים מוגרלים. כולל התאמת "בדיקת המסלול" (`simulatePath`) להליכה האמיתית |
| `src/admin/diagram-vs-form.test.tsx` | כל 38 המסכים מרונדרים כמו שהמשיב רואה אותם, ומושווים לטקסטים שהקונסולה מציגה (צומת, שם מסך, שם תשובה) |
| `src/admin/publish-guard.test.tsx` | שינויים לא שמורים ⇒ "שמירה ופרסום"; שמירה שנכשלה ⇒ **לא מפרסמים**; שגיאות בדיקה ⇒ הכפתור חסום |
| `netlify/functions/__tests__/admin-publish.test.ts` | מה שנכתב ל-`survey_configs` הוא הטיוטה של אותו שאלון, כמות שהיא, עם `version` חדש בלבד; שאלון שבור נחסם ב-422 ולא נכתב דבר |
| `netlify/functions/__tests__/config-get.test.ts` | איזה שאלון מוגש: הגרסה האחרונה של ה-slug, גרסה מוצמדת לסשן, 410 לשאלון מאורכב |
| `src/data/config-pinning.test.ts` | משיב באמצע נשאר בגרסה שלו גם אחרי פרסום |
| `netlify/functions/__tests__/admin-draft.test.ts`, `session.test.ts` | הטיוטה נשמרת בדיוק כפי שנשלחה, עם נעילה אופטימית; וכל קצה אדמין חוסם מי שאינו עורך `first-edea.com` **לפני** כל גישה ל-DB |

הרצת קובץ אחד: `npx vitest run src/data/journey.test.tsx`.

**מה עוד לא מכוסה:** `supabase/schema.sql` נבדק כטקסט בלבד — אף בדיקה לא מריצה
Postgres אמיתי, ולכן טעות בהגדרת view או ב-`check` תתגלה רק בהרצה בדשבורד.
כך גם זרימת ה-OAuth האמיתית מול Google, והמראה בדפדפן (לזה יש `qa/`).
